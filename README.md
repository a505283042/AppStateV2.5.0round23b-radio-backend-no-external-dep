...
本项目采用“源码公开、禁止商用、商业授权另议”的许可方式。

- 原创固件、Web前端和工具脚本：**PolyForm Noncommercial License 1.0.0**，见 `LICENSE`。
- 原创PCB、原理图、外壳、机械结构和文档：**CC BY-NC-SA 4.0**，见 `LICENSE-HARDWARE-DOCS.md`。
- 第三方库和框架：继续遵循各自原始许可证，见 `THIRD_PARTY_NOTICES.md`。
- 商业使用：销售整机、PCB、套件、固件，或用于收费项目、商业交付和商业部署前，必须取得项目所有者单独的书面授权，见 `COMMERCIAL-LICENSE.md`。

允许个人学习、研究、实验、非商业制作、修改和展示。分发修改版本时必须保留版权、许可证和第三方声明。

由于公开许可证禁止商业使用，本项目属于 **Source Available（源码公开）**，不属于 OSI 定义下的开源软件。
